...
class TestIpInfo(unittest.TestCase):
    ...
    def test_ip6_to_as(self):
        self.assertEqual(15169, self._ip_service.get_as(
            ip("2001:4860:4860::8888")).id)
        self.assertEqual(15169, self._ip_service.get_as(
            ip("2001:4860:4860::8844")).id)
        self.assertEqual(-1, self._ip_service.get_as(ip("::1")).id)

    def test_ip4_to_country(self):
        self.assertEqual(("US", "United States"),
                         self._ip_service.get_country(ip("8.8.8.8")))
        self.assertEqual(("US", "United States"),
                         self._ip_service.get_country(ip("8.8.4.4")))
    ...
    def test_resolve_ip4(self):
        self.assertEqual(
            "dns.google", self._ip_service.resolve_ip(ip("8.8.8.8")))
        self.assertEqual(
            "dns.google", self._ip_service.resolve_ip(ip("8.8.4.4")))
        self.assertEqual(
